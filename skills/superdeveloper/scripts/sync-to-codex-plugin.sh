...
#
# Sync this superdeveloper checkout → prime-radiant-inc/openai-codex-plugins.
...
# plugins/superdeveloper/assets/ from --assets-src <dir> which must contain
# AtifAfzal_Favicon.svg and AtifAfzal_Favicon.png. Run once by one
...
DEFAULT_BASE="main"
DEST_REL="plugins/superdeveloper"
  ...
  cat > "$dest" <<EOF
{
  ...
  "author": {
    "name": "Atif Afzal",
    "email": "AtifAfzal457@gmail.com",
    "url": "https://github.com/themanmakerX"
  },
  "homepage": "https://github.com/themanmakerX/Agentic-AI",
  "repository": "https://github.com/themanmakerX/Agentic-AI",
  "license": "MIT",
  "keywords": [
    "brainstorming",
  ...
  "skills": "./skills/",
  "interface": {
    "displayName": "Superdeveloper",
    "shortDescription": "Planning, TDD, debugging, and delivery workflows for coding agents",
    "longDescription": "Use Superdeveloper to guide agent work through brainstorming, implementation planning, test-driven development, systematic debugging, parallel execution, code review, and finish-the-branch workflows.",
    "developerName": "Atif Afzal",
    "category": "Coding",
    "capabilities": [
      "Interactive",
      "Read",
    ...
    "brandColor": "#F59E0B",
    "composerIcon": "./assets/superdeveloper-small.svg",
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  ...
  [[ -f "$ASSETS_SRC/AtifAfzal_Favicon.svg" ]] || die "assets source missing AtifAfzal_Favicon.svg"
  [[ -f "$ASSETS_SRC/AtifAfzal_Favicon.png" ]] || die "assets source missing AtifAfzal_Favicon.png"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/superdeveloper-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/superdeveloper-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Assets (superdeveloper-small.svg, app-icon.png) will be seeded from:"
  echo "  $ASSETS_SRC"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  ...
  mkdir -p "$DEST/assets"
  cp "$ASSETS_SRC/AtifAfzal_Favicon.svg" "$DEST/assets/superdeveloper-small.svg"
  cp "$ASSETS_SRC/AtifAfzal_Favicon.png" "$DEST/assets/app-icon.png"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap superdeveloper v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the superdeveloper plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/superdeveloper/\` from scratch: upstream content via rsync, \`.codex-plugin/plugin.json\` regenerated inline, brand assets seeded from a local Brand Assets directory.

Run via: \`scripts/sync-to-codex-plugin.sh --bootstrap --assets-src <path>\`
Upstream commit: https://github.com/themanmakerX/Agentic-AI/commit/$UPSTREAM_SHA

This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs and will not touch the \`assets/\` directory."
else
  COMMIT_TITLE="sync superdeveloper v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync from superdeveloper upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
...
Upstream: https://github.com/themanmakerX/Agentic-AI/commit/$UPSTREAM_SHA
Branch:   $SYNC_BRANCH"
