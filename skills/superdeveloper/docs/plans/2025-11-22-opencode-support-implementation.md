...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superdeveloper:executing-plans to implement this plan task-by-task.

**Goal:** Add full superdeveloper support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/superdeveloper-codex` (lines 40-74)
...
- Reference: `.codex/superdeveloper-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'superdeveloper' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superdeveloper-codex` (lines 212-280)
 ...
 * (personal skills override superdeveloper skills).
 *
 * @param {string} skillName - Name like "superdeveloper:brainstorming" or "my-skill"
 * @param {string} superdeveloperDir - Path to superdeveloper skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superdeveloperDir, personalDir) {
    // Strip superdeveloper: prefix if present
    const forceSuperdeveloper = skillName.startsWith('superdeveloper:');
    const actualSkillName = forceSuperdeveloper ? skillName.replace(/^superdeveloper:/, '') : skillName;

    // Try personal skills first (unless explicitly superdeveloper:)
    if (!forceSuperdeveloper && personalDir) {
    ...
    // Try superdeveloper skills
    if (superdeveloperDir) {
        const superdeveloperPath = path.join(superdeveloperDir, actualSkillName);
        const superdeveloperSkillFile = path.join(superdeveloperPath, 'SKILL.md');
        if (fs.existsSync(superdeveloperSkillFile)) {
            return {
                skillFile: superdeveloperSkillFile,
                sourceType: 'superdeveloper',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superdeveloper-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/superdeveloper-codex` (add import at top)
...
Run: `node -c .codex/superdeveloper-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/superdeveloper-codex
...
**Files:**
- Modify: `.codex/superdeveloper-codex` (lines 40-74)
...
Run: `.codex/superdeveloper-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/superdeveloper-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/superdeveloper-codex` (lines 16-38, approximately)
...
Run: `.codex/superdeveloper-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/superdeveloper.js`
...
/**
 * Superdeveloper plugin for OpenCode.ai
 *
...
const superdeveloperSkillsDir = path.join(homeDir, '.config/opencode/superdeveloper/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SuperdeveloperPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/superdeveloper.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/superdeveloper.js
...
**Files:**
- Modify: `.opencode/plugin/superdeveloper.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superdeveloper:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > superdeveloper)
          const resolved = skillsCore.resolveSkillPath(
            skill_name,
            superdeveloperSkillsDir,
            personalSkillsDir
          );
...
Run: `node -c .opencode/plugin/superdeveloper.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the superdeveloper and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const superdeveloperSkills = skillsCore.findSkillsInDir(
            superdeveloperSkillsDir,
            'superdeveloper',
            3
          );
          ...
          const allSkills = [...personalSkills, ...superdeveloperSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install superdeveloper skills to ~/.config/opencode/superdeveloper/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superdeveloper:';
...
```javascript
    'session.started': async () => {
      // Read using-superdeveloper skill content
      const usingSuperdeveloperPath = skillsCore.resolveSkillPath(
        'using-superdeveloper',
        superdeveloperSkillsDir,
        personalSkillsDir
      );

      let usingSuperdeveloperContent = '';
      if (usingSuperdeveloperPath) {
        const fullContent = fs.readFileSync(usingSuperdeveloperPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSuperdeveloperContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Superdeveloper skills: \`superdeveloper:skill-name\` (from ~/.config/opencode/superdeveloper/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/superdeveloper')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/superdeveloper && git pull` to update superdeveloper.' :
        '';

      // Return context to inject into session
      return {
        context: `<EXTREMELY_IMPORTANT>
You have superdeveloper.

**Below is the full content of your 'superdeveloper:using-superdeveloper' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSuperdeveloperContent}
...
### 1. Install Superdeveloper Skills

```bash
# Clone superdeveloper skills to OpenCode config directory
mkdir -p ~/.config/opencode/superdeveloper
git clone https://github.com/themanmakerX/Agentic-AI.git ~/.config/opencode/superdeveloper
```

### 2. Install the Plugin

The plugin is included in the superdeveloper repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/superdeveloper/.opencode/plugin/superdeveloper.js`
...
ln -s ~/.config/opencode/superdeveloper/.opencode/plugin/superdeveloper.js .opencode/plugin/superdeveloper.js
```
...
```
use use_skill tool with skill_name: "superdeveloper:brainstorming"
```
...
Personal skills override superdeveloper skills with the same name.

## Updating

```bash
cd ~/.config/opencode/superdeveloper
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/superdeveloper/.opencode/plugin/superdeveloper.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/superdeveloper/skills`
...
- Documentation: https://github.com/themanmakerX/Agentic-AI
```
...
Superdeveloper works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
**Files:**
- Test: `.codex/superdeveloper-codex`
...
Run: `.codex/superdeveloper-codex use-skill superdeveloper:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/superdeveloper-codex bootstrap | head -30`
...
ls -l .opencode/plugin/superdeveloper.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/superdeveloper.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/superdeveloper-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/superdeveloper-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/superdeveloper.js` created with tools and hooks
