...
Add full superdeveloper support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port superdeveloper to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/superdeveloper-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/superdeveloper.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/superdeveloper/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/superdeveloper-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "superdeveloper:brainstorming")')
  }),
...
1. **Inject using-superdeveloper content**
   - Full content of the using-superdeveloper skill
...
```javascript
// .opencode/plugin/superdeveloper.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const SuperdeveloperPlugin = async ({ client, directory, $ }) => {
  const superdeveloperDir = path.join(process.env.HOME, '.config/opencode/superdeveloper');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSuperdeveloper = await readSkill('using-superdeveloper');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingSuperdeveloper}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── superdeveloper-codex        # UPDATED: Use skills-core
│   ├── superdeveloper-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── superdeveloper.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/superdeveloper-codex`
...
2. Update `.codex/superdeveloper-codex` to use shared core
...
1. Create `.opencode/plugin/superdeveloper.js`
