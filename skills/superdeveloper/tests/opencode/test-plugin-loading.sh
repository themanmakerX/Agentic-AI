#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the superdeveloper plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/superdeveloper.js"
...
skill_count=$(find "$SUPERDEVELOPER_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SUPERDEVELOPER_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-superdeveloper skill exists (critical for bootstrap)
echo "Test 3: Checking using-superdeveloper skill (required for bootstrap)..."
if [ -f "$SUPERDEVELOPER_SKILLS_DIR/using-superdeveloper/SKILL.md" ]; then
    echo "  [PASS] using-superdeveloper skill exists"
else
    echo "  [FAIL] using-superdeveloper skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SUPERDEVELOPER_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/superdeveloper/' "$SUPERDEVELOPER_PLUGIN_FILE"; then
