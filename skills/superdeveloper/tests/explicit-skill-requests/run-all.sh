...
# Test: please use superdeveloper
echo ">>> Test 0: please-use-superdeveloper"
if "$SCRIPT_DIR/run-test.sh" "superdeveloper" "$PROMPTS_DIR/please-use-superdeveloper.txt"; then
    PASSED=$((PASSED + 1))
    RESULTS="$RESULTS\nPASS: please-use-superdeveloper"
else
    FAILED=$((FAILED + 1))
    RESULTS="$RESULTS\nFAIL: please-use-superdeveloper"
fi
echo ""
