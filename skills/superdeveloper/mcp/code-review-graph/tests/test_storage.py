...
from code_review_graph.incremental import IncrementalIndexer
from code_review_graph.languages import build_default_registry
from code_review_graph.parser import EdgeRecord, GraphParser, ParseDiagnostic, ParseResult, ParserBackend, Span, Position, SymbolRecord
from code_review_graph.storage import SQLiteGraphStore, WorkspaceFileSnapshot
...
def test_storage_replaces_file_graph(sample_repo_path) -> None:
    db_path = Path(r"C:\Users\acer\.codex\memories\code-review-graph-storage.sqlite")
...
def test_incremental_index_updates_only_changed_files(sample_repo_path) -> None:
    ...
    db_path = Path(r"C:\Users\acer\.codex\memories\code-review-graph-incremental.sqlite")
    db_path.unlink(missing_ok=True)
    store = SQLiteGraphStore(db_path)
