...
from .runtime import public_graph_root, root_slug, workspace_db_path
...
class GraphService:
    ...
    def build(self, progress_callback=None) -> BuildSummary:
        return build_graph(self.root_path, self.db_path, progress_callback=progress_callback)

    def update(self, progress_callback=None) -> UpdateSummary:
        return update_graph(self.root_path, self.db_path, progress_callback=progress_callback)
    ...
    def autocomplete_entities(self, prefix: str, limit: int = 20) -> dict[str, Any]:
        prefix = prefix.strip()
        needle = prefix.lower()
        with self._store() as store:
            files = store.fetch_file_rows()
        ...
        for file_row in files:
            labels = [
                ...
                str(file_row.get("module_name", "")),
                str(file_row.get("language_id", "")),
            ]
            haystack = " ".join(labels).lower()
            if not needle or needle in haystack:
                score = 0 if haystack.startswith(needle) else 1
                candidates.append(
                    {
                        "type": "file",
                        "id": file_row.get("id"),
                        "label": file_row.get("path") or file_row.get("module_name") or f'file:{file_row.get("id")}',
                        "path": file_row.get("path"),
                        "module_name": file_row.get("module_name"),
                        "language_id": file_row.get("language_id"),
                        "score": score,
                    }
                )
        ...
        for symbol in symbols:
            ...
            haystack = " ".join(labels).lower()
            if not needle or needle in haystack:
                file_row = next((row for row in files if row["id"] == symbol.get("file_id")), None)
                score = 0 if haystack.startswith(needle) else 1
                candidates.append(
                    {
                        "type": "symbol",
                        "id": symbol.get("id"),
                        "label": symbol.get("qualified_name") or symbol.get("name") or f'symbol:{symbol.get("id")}',
                        "name": symbol.get("name"),
                        "qualified_name": symbol.get("qualified_name"),
                        "kind": symbol.get("kind"),
                        "path": file_row["path"] if file_row else None,
                        "file_id": symbol.get("file_id"),
                        "language_id": file_row["language_id"] if file_row else None,
                        "score": score,
                    }
                )

        candidates.sort(key=lambda item: (int(item.get("score", 0)), str(item.get("type", "")), str(item.get("label", ""))))
        results = candidates[:limit]
        return {
            "prefix": prefix,
            "limit": limit,
            "results": results,
            "suggestions": [str(item.get("label", "")) for item in results],
        }
    ...
    def get_neighbors(self, targets: list[str], depth: int = 1) -> dict[str, Any]:
        with self._store() as store:
            files = {str(row["id"]): row for row in store.fetch_file_rows()}
            edges = store.fetch_edges()
        ...
        for edge in edges:
            ...
            source = str(source_file_id)
            target = str(target_file_id)
            if source in impacted or target in impacted:
                selected_edges.append(
                    {
                        **edge,
                        "source_path": str(files[source]["path"]) if source in files else None,
                        "target_path": str(files[target]["path"]) if target in files else None,
                    }
                )

        neighbor_rows = []
        for file_id in sorted(impacted):
            ...
            neighbor_rows.append(
                {
                    **row,
                    "is_seed": file_id in seed_ids,
                    "incoming": sum(1 for edge in selected_edges if str(edge.get("target_file_id")) == file_id),
                    "outgoing": sum(1 for edge in selected_edges if str(edge.get("source_file_id")) == file_id),
                }
            )
        ...
        return {
            "targets": targets,
            "seed_paths": [str(files[file_id]["path"]) for file_id in seed_ids if file_id in files],
            "neighbor_paths": [str(row["path"]) for row in neighbor_rows],
    ...
    def find_paths(self, source: str, sink: str, max_depth: int = 4) -> dict[str, Any]:
        return self.trace_dataflow(source=source, sink=sink, max_depth=max_depth)
    ...
    def ask_graph(self, question: str, limit: int = 8, depth: int = 2) -> dict[str, Any]:
        question = question.strip()
        matches = self._fast_question_matches(question=question, limit=limit)
        targets: list[str] = []
        for match in matches:
            if match.get("type") == "file" and match.get("path"):
                targets.append(str(match["path"]))
            elif match.get("type") == "symbol":
                if match.get("file_path"):
                    targets.append(str(match["file_path"]))
                elif match.get("qualified_name"):
                    targets.append(str(match["qualified_name"]))
                elif match.get("name"):
                    targets.append(str(match["name"]))
        targets = list(dict.fromkeys(targets))[:3]
        neighbors = self.get_neighbors(targets=targets, depth=depth) if targets else {"neighbors": [], "edges": []}
        paths: list[dict[str, Any]] = []
        if len(targets) >= 2:
            path_data = self.find_paths(targets[0], targets[1], max_depth=max(depth + 1, 4))
            paths = [{"source": targets[0], "sink": targets[1], **path_data}]

        top_labels = [str(match.get("path") or match.get("qualified_name") or match.get("label") or match.get("name") or "") for match in matches[:3]]
        answer_bits = []
        if top_labels:
            answer_bits.append(f"Top related entities: {', '.join(top_labels)}.")
        if neighbors.get("neighbors"):
            answer_bits.append(f"Neighborhood expansion found {len(neighbors['neighbors'])} nearby files.")
        if paths:
            answer_bits.append("A direct path candidate was found between the top retrieved entities.")
        if not answer_bits:
            answer_bits.append("I could not find a strong match, but the graph is indexed and ready for targeted search.")
        ...
        return {
            ...
            "suggested_followups": [
                "Refine the search to a specific file or symbol name.",
                "Use get_neighbors on a selected target to expand the local subgraph.",
                "Use find_paths with a source and sink to trace dependency flow.",
            ],
        }

    def _fast_question_matches(self, question: str, limit: int) -> list[dict[str, Any]]:
        ...
        focus_tokens = [token for token in query_tokens if token not in stop_words]
        search_text = " ".join(focus_tokens) if focus_tokens else question

        auto = self.autocomplete_entities(search_text, limit=max(limit * 2, 12))
        auto_results = auto.get("results", [])
        if auto_results:
            return auto_results[:limit]

        pattern_hits = self.find_files_by_pattern(search_text, limit=limit).get("matches", [])
        if pattern_hits:
            return [{"type": "file", **row, "label": row.get("path")} for row in pattern_hits]

        query_matches = self.query(search_text, limit=limit).get("matches", [])
        if query_matches:
        ...
        for file_row in files:
            ...
            overlap = len([token for token in focus_tokens if token in haystack]) if focus_tokens else int(question_lower in haystack)
            if overlap:
                scored.append(
                    {
                        "type": "file",
                ...
                scored.append(
                    {
                        ...
                        "name": symbol.get("name"),
                        "qualified_name": symbol.get("qualified_name"),
                        "label": symbol.get("qualified_name") or symbol.get("name"),
                        "file_id": symbol.get("file_id"),
                        "score": overlap,
                    }
                )

        scored.sort(key=lambda item: (-int(item.get("score", 0)), str(item.get("type", "")), str(item.get("label", ""))))
        return scored[:limit]
    ...
    def generate_wiki(self, write_to_disk: bool = False) -> dict[str, Any]:
        ...
        output_dir = public_graph_root() / "wiki" / root_slug(self.root_path)
        if write_to_disk:
...
def create_service(root_path: Path, db_path: Path | None = None) -> GraphService:
    root_path = Path(root_path).resolve()
    resolved_db_path = Path(db_path).resolve() if db_path is not None else workspace_db_path(root_path)
    return GraphService(root_path=root_path, db_path=resolved_db_path)
